...
const createThreadsSlice: StoreSlice<ThreadsSlice, AuthSlice> = (set, get) => ({
    ...
    loadPreviews: async (board: Board, page = 0) => {
        ...
        if (!get().loadedBoards.includes(board.id)) {
            set((state) => ({
                loadedBoards: [...state.loadedBoards, board.id],
            }));
        }

        set((state) => ({
            threads: [
                ...state.threads.filter((t) => t.board !== board.id),
                ...threads,
            ],
        }));
    },
    refreshPreviews: async (board: Board) => {
        set(({ loadedBoards }) => ({
            loadedBoards: loadedBoards.filter((id) => id !== board.id),
        }));
    },
    createThread: async (
        board: Board,
    ...
    deletePost: async (thread: Thread, post: Post) => {
        ...
        set((state) => ({
            threads: [
                ...
                {
                    ...thread,
                    posts: thread.posts.filter((p) => p.id !== post.id),
                },
            ],
        }));
    },

...
export const renderPostLinks = (text: string) => {
    ...
    const substrings = [...newText.matchAll(/>>([0-9]+)|.+?(?=>>([0-9]+)|(\n|\r)|$)+|\n|\r/g)];
    return substrings
        .map((matchArray) => {
...
export const renderQuotes = (elements: (string | JSX.Element)[]) => elements
    // Each array item is one line, this places <br />s into them correctly
    .reduce<(string | JSX.Element)[][]>((result, element) => {
        ...
        if (currentLineLast && typeof currentLineLast !== "string" && currentLineLast.type === "br") {
            result.push([]);

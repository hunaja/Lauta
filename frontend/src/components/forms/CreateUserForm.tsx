...
export default function CreateUserForm() {
    ...
    const handleFormSubmit: SubmitHandler<FieldValues> = (data) => {
        createUser({
            ...data,
            role: roles.find((r) => r.pretty === data.role)?.name ?? defaultRole.pretty,
        } as UserForm)
            .then(() => reset());
    };
    ...
    return (
        <div className="p-1">
            <form onSubmit={handleSubmit(handleFormSubmit)}>
                ...
                <input {...register("password", { required: true })} type="password" className="w-full m-1 border-2 border-purple-200 focus:outline-none focus:border-purple-400 p-1" placeholder="Salasana" />
                <br />
                <input
                    {

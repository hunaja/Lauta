...
router.post("/:id/replies", uploadMiddleware, async (req, res) => {
    ...
    if (author?.length < 3) return res.status(400).json({ error: "Käyttäjännimi on liian lyhyt.", field: "author" });
    if (!text && !req.file) return res.status(400).json({ error: "Joko kuva tai tiedosto on annettava.", field: "text" });
